...
def mostrarMinas(tableroCliente, minasPosiciones):
    # primero obtenemos las posiciones
    contadorPos = 0
    posX = 0
    posY = 0
    for i in range(len(minasPosiciones)):
        # print("La minaPos: ",minasPosiciones[i])
        if (minasPosiciones[i].isalnum()) and contadorPos == 0:
            posX = int(minasPosiciones[i])
            contadorPos+=1
            i+=1
            # print(">>>La minaPos: ",minasPosiciones[i])
        if (minasPosiciones[i].isalnum()) and contadorPos ==1 :
            posY = int(minasPosiciones[i])
            contadorPos +=1
            i+=1
            # print(">>>La minaPos: ",minasPosiciones[i])
        if contadorPos >= 2:
            tableroCliente[posX][posY] = "M"
            # print(">>>La minaPosXY: ",tableroCliente[posX][posY])
            contadorPos = 0
    print("\n            ***** El Tablero con las minas ***** ")
    imprimirTablero(tableroCliente)
    ...
    ***************** Aplicación Cliente *****************
    ...
    while True:
        ...
        if mensajeRecibido.decode(encoding="ascii", errors="ignore") == "True":
            ...
            #recibo la ubicacion de las minas 
            mensajeRecibido = TCPClientSocket.recv(buffer_size)
            # print("Servidor (posMinas): ", mensajeRecibido.decode(encoding="ascii", errors="ignore"))
            #decodificamos las posiciones y le damos su tratamiento para mostrarlas en pantalla
            minasPosiciones = mensajeRecibido.decode(encoding="ascii", errors="ignore")
            mostrarMinas(tableroCliente, minasPosiciones)
            #Recibo la duracion del juego
